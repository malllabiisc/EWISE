...
#
#Modified by Sawan Kumar: Accept encodings of synsets as inputs
#
...
def get_encodings(model, e1, tokenidx_to_synset, get_definitions):
    e1_tokens = [tokenidx_to_synset[idx] for idx in e1.detach().cpu().numpy().ravel()]
    batch, lengths = get_definitions(e1_tokens)
    batch = batch.cuda()
    e1_emb = model.encoder((batch, lengths))[0]
    return e1_emb

def ranking_and_hits(model, dev_rank_batcher, vocab, name, tokenidx_to_synset=None, get_definitions=None):
    log.info('')
    log.info('-'*50)
    log.info(name)
    log.info('-'*50)
    ...
    for i, str2var in tqdm(enumerate(dev_rank_batcher)):
        ...
        if tokenidx_to_synset is not None:
            e1_emb = get_encodings(model, e1, tokenidx_to_synset, get_definitions)
            e2_emb = get_encodings(model, e2, tokenidx_to_synset, get_definitions)
            pred1 = model.forward(e1_emb, rel, e1_encoded=True)
            pred2 = model.forward(e2_emb, rel_reverse, e1_encoded=True)
        else:
